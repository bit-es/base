...
class Setting extends Model
{
    ...
    public static function get(string $key, $scope = null)
    {
        if ($scope && is_object($scope)) {
            ...
            if ($s) return static::castValue($s->value);
    ...
    protected static function castValue($value)
    {
        ...
        if (is_numeric($value)) {
            if (strpos((string)$value, '.') !== false) return (float)$value;
            return (int)$value;
        }
        return $value;
    }

    public static function forGlobal($key) { return static::where('key',$key)->whereNull('scope_type')->first(); }
    public static function forModel($key, $model) { return static::where('key',$key)->where('scope_type','model')->where('scope_id',$model->getKey())->first(); }
}
